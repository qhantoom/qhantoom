...
impl<'a> Parser<'a> {
  ...
  fn parse_local_stmt(&mut self) -> Result<Box<Stmt>, String> {
    ...
    self.expect_first(&TokenKind::EqEq)?;
    self.next();
  ...
  fn parse_ty(&mut self) -> Result<Box<Ty>, String> {
    ...
    let kind = match self.current.kind() {
      ...
      TokenKind::CharAscii(..) => Ok(TyKind::Char),
      TokenKind::FloatNumber(..) => Ok(TyKind::Float),
      TokenKind::IntNumber(..) => Ok(TyKind::Int),
      TokenKind::StrBuffer(..) => Ok(TyKind::Str),
