...
use super::token::{
  Token, TokenKind, NUMBER_BASE_BIN, NUMBER_BASE_DEC, NUMBER_BASE_HEX,
  NUMBER_BASE_OCT, TOKEN_EOF,
};
  ...
  fn step(&mut self, c: char) -> Option<TokenKind> {
    ...
    match self.state {
      ...
      TokenizerState::Idle => match c {
        ...
        c if is_number_continue(c) => go!(self: push c; to NumberDec),
      ...
      TokenizerState::StartNumber => match c {
        ...
        'b' => go!(self: push c; to NumberBin),
        'o' => go!(self: push c; to NumberOct),
        '1'..='9' => self.err(Error::UnexpectedLiteralNumber(c)),
        '.' => go!(self: push c; to NumberFloat),
        c => go!(self: emit_number_zero c),
      },
      // read_number_state
      TokenizerState::NumberDec => match c {
        '0'..='9' | '_' => go!(self: push c),
        '.' => go!(self: push c; to NumberFloat),
        c => go!(self: emit_number_dec c),
      },
      // read_number_float_state
      TokenizerState::NumberFloat => match c {
        '0'..='9' | '_' => go!(self: push c),
        c => go!(self: emit_number_float c),
      },
      // read_number_oct_state
      TokenizerState::NumberOct => match c {
        '0'..='7' | '_' => go!(self: push c),
        c => go!(self: emit_number_oct c),
      },
      // read_number_hex_state
      TokenizerState::NumberHex => match c {
        '0'..='9' | 'a'..='f' | 'A'..='F' => go!(self: push c),
        c => go!(self: emit_number_hex c),
      },
      // read_number_bin_state
      TokenizerState::NumberBin => match c {
        '0'..='1' | '_' => go!(self: push c),
        c => go!(self: emit_number_bin c),
      },
...
macro_rules! go (
  ...
  ( $me:ident : emit_number_zero $c:ident ) => ({
    $me.buffer.clear();
  ...
  ( $me:ident : emit_number_dec $c:expr) => ({
    let num = $me.buffer.replace("_", "").to_owned();
    let num = i64::from_str_radix(&num, NUMBER_BASE_DEC);
    ...
    match num {
      ...
      Err(_) => $me.err(Error::Custom("invalid int literal (need digits)")),
    }
  });

  ( $me:ident : emit_number_float $c:expr) => ({
    let num = $me.buffer.replace("_", "").to_owned();
  ...
  ( $me:ident : emit_number_bin $c:expr) => ({
    if $me.buffer.len() == 0 {
      $me.err(Error::Custom("invalid binary literal (need digits)"));
    }

    let buf = $me.buffer.replace("_", "").to_owned();
    let without_prefix = buf.trim_start_matches("0b");
    let num = i64::from_str_radix(&without_prefix, NUMBER_BASE_BIN);
    ...
    match num {
      ...
      Err(_) => $me.err(Error::Custom("invalid binary literal (need digits)")),
    }
  });

  ( $me:ident : emit_number_oct $c:expr) => ({
    if $me.buffer.len() == 0 {
      $me.err(Error::Custom("invalid oct literal (need digits)"));
    }

    let buf = $me.buffer.replace("_", "").to_owned();
    let without_prefix = buf.trim_start_matches("0o");
    let num = i64::from_str_radix(&without_prefix, NUMBER_BASE_OCT);
    ...
    match num {
      ...
      Err(_) => $me.err(Error::Custom("invalid oct literal (need digits)")),
    }
  });

  ( $me:ident : emit_number_hex $c:expr) => ({
    if $me.buffer.len() == 0 {
    ...
    let num = i64::from_str_radix(without_prefix, NUMBER_BASE_HEX);
